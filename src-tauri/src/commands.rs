...
/// 导出自绘断面线/点为 Shapefile（UTF-8 编码 + .cpg，中文属性不乱码）。
/// base_path 为基名（可带 .shp 后缀，自动去除）：写 `<base>-断面线.shp` 与 `<base>-断面点.shp`（WGS84）
#[tauri::command]
pub async fn export_shp(base_path: String, name: String,
                        line: Vec<(f64, f64)>, points: Vec<(f64, f64, String)>) -> Result<Value, String> {
    use crate::gdal_ffi::{self, OFT_STRING, WKB_LINESTRING, WKB_POINT};
    use std::ffi::CString;
    let g = gdal_ffi::get()?;
    if line.len() < 2 && points.is_empty() {
        return Err("无可导出的断面线/点".into());
    }
    let base = base_path.trim_end_matches(".shp").to_string();
    if let Some(dir) = std::path::Path::new(&base).parent() {
        let _ = std::fs::create_dir_all(dir);
    }
    unsafe {
        let drv = (g.f.ogr_get_driver_by_name)(b"ESRI Shapefile ".as_ptr());
        if drv.is_null() { return Err("GDAL 未提供 ESRI Shapefile 驱动".into()); }
        let srs = (g.f.osr_new_srs)(std::ptr::null());
        (g.f.osr_import_from_epsg)(srs, 4326);
        (g.f.osr_set_axis_mapping)(srs, 0);
        let enc = b"ENCODING=UTF-8 ";
        let opts: [*const u8; 2] = [enc.as_ptr(), std::ptr::null()];
        let mut wrote: Vec<String> = Vec::new();
        let write_layer = |pth: &str, lname: &str, wkb: u32, fields: &[&str], feats: &[(Vec<(f64, f64)>, Vec<String>)]| -> Result<(), String> {
            let path_c = CString::new(pth.to_string()).map_err(|_| "路径含非法字符".to_string())?;
            let ds = (g.f.ogr_dr_create_ds)(drv, path_c.as_ptr() as *const u8, std::ptr::null());
            if ds.is_null() { return Err(format!("创建 SHP 失败: {}", g.last_error())); }
            let lname_c = CString::new(lname).unwrap();
            let layer = (g.f.ogr_ds_create_layer)(ds, lname_c.as_ptr() as *const u8, srs, wkb, opts.as_ptr());
            if layer.is_null() { (g.f.ogr_ds_destroy)(ds); return Err(format!("创建图层失败: {}", g.last_error())); }
            let defn = (g.f.ogr_l_get_defn)(layer);
            for fname in fields {
                let fc = CString::new(*fname).unwrap();
                let fd = (g.f.ogr_fld_create)(fc.as_ptr() as *const u8, OFT_STRING);
                (g.f.ogr_fld_set_width)(fd, 254);
                (g.f.ogr_l_create_field)(layer, fd, 1);   // GDAL3 layer defn 已 sealed：字段经 OGR_L_CreateField 添加
            }
            for (coords, attrs) in feats {
                let f = (g.f.ogr_f_create)(defn);
                for (i, a) in attrs.iter().enumerate() {
                    let ac = CString::new(a.as_str()).unwrap_or_default();
                    (g.f.ogr_f_set_field_string)(f, i as i32, ac.as_ptr() as *const u8);
                }
                let geom = (g.f.ogr_g_create_geometry)(wkb);
                for (x, y) in coords { (g.f.ogr_g_add_point_2d)(geom, *x, *y); }
                (g.f.ogr_f_set_geometry_directly)(f, geom);
                let rc = (g.f.ogr_l_create_feature)(layer, f);
                (g.f.ogr_f_destroy)(f);
                if rc != 0 { (g.f.ogr_ds_destroy)(ds); return Err(format!("写入要素失败: {}", g.last_error())); }
            }
            (g.f.ogr_ds_destroy)(ds);
            Ok(())
        };
        if line.len() >= 2 {
            // ESRI Shapefile 驱动要求数据源路径带 .shp 后缀（否则创建为目录型数据源）
            let pth = format!("{}-断面线.shp", base);
            write_layer(&pth, "line", WKB_LINESTRING, &["profile", "kind"], &[(line.clone(), vec![name.clone(), "断面线".to_string()])])?;
            wrote.push(pth);
        }
        if !points.is_empty() {
            let pth = format!("{}-断面点.shp", base);
            let feats: Vec<(Vec<(f64, f64)>, Vec<String>)> = points.iter().enumerate()
                .map(|(i, (x, y, nm))| (vec![(*x, *y)], vec![name.clone(), format!("点{}", i + 1), nm.clone()]))
                .collect();
            write_layer(&pth, "point", WKB_POINT, &["profile", "no", "name"], &feats)?;
            wrote.push(pth);
        }
