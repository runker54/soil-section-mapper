...
fns! {
    ...
    ogr_get_driver_by_name = OGRGetDriverByName : fn (*const u8) -> *mut c_void;
    ogr_dr_create_ds = OGR_Dr_CreateDataSource : fn (*mut c_void, *const u8, *const *const u8) -> *mut c_void;
    ogr_ds_create_layer = OGR_DS_CreateLayer : fn (*mut c_void, *const u8, HSRS, u32, *const *const u8) -> HLayer;
    ogr_ds_destroy = OGR_DS_Destroy : fn (*mut c_void) -> ();
    ogr_fld_create = OGR_Fld_Create : fn (*const u8, u32) -> *mut c_void;
    ogr_fld_set_width = OGR_Fld_SetWidth : fn (*mut c_void, i32) -> ();
    ogr_fd_add_field_defn = OGR_FD_AddFieldDefn : fn (*mut c_void, *mut c_void) -> ();
    ogr_f_create = OGR_F_Create : fn (*mut c_void) -> HFeature;
    ogr_f_set_field_string = OGR_F_SetFieldString : fn (HFeature, i32, *const u8) -> ();
    ogr_f_set_geometry_directly = OGR_F_SetGeometryDirectly : fn (HFeature, HGeom) -> i32;
    ogr_l_create_feature = OGR_L_CreateFeature : fn (HLayer, HFeature) -> i32;
    ogr_l_create_field = OGR_L_CreateField : fn (HLayer, *mut c_void, i32) -> i32;
