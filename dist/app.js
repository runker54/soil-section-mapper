...
function renderPointsTable() {
  ...
  S.table.forEach((r, i) => {
    html += `<tr data-i="${i}"><td class="grip"><span class="gripd" title="拖动调整顺序（里程随之重新分配）">⠿</span></td>` + PT_COLS.map(c2 =>
...
async function autoFillRowFromChainage(rowIdx, km) {
  ...
  for (let i = 0; i < line.length - 1; i++) {
    const d = Math.hypot(line[i + 1][0] - line[i][0], line[i + 1][1] - line[i][1]) * 111320;
    segs.push({ a: line[i], b: line[i + 1], len: d });
    total += d;
  }
  let target = Math.min(Math.max(km * 1000, 0), total);
  let cur = 0, si = 0;
  while (si < segs.length - 1 && cur + segs[si].len < target) { cur += segs[si].len; si++; }
  const seg = segs[si];
  const t = Math.max(0, Math.min(1, (target - cur) / (seg.len || 1)));
...
function placePointsAlongLine(intervalKm) {
  ...
  const step = Math.max(0.05, intervalKm) * 1000;
...
$("btnAddPoint").onclick = async () => {
  if (S.useCustom) {
    ...
    const pr = currentProfile();
    const p0 = line[0];
    pr.drawPts.push([p0[0], p0[1], "点" + (pr.drawPts.length + 1)]);
    S.map.pts = pr.drawPts.slice();
    updateDrawSources();
    try { await commitCustomPoints(); } catch (e) { log("点提交失败: " + e, "err"); }
    computeDebounced();
    log(`已在「${pr.name}」起点新增点${pr.drawPts.length}：表格将自动出现该行，可填写里程定位`);
    return;
  }
