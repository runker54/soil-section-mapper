...
/* 页脚版权（年份取当前年）：主页页脚归位到 #app 末尾（避开弹窗嵌套），版权居中；
   断面绘图页页脚同样居中展示（绝对定位，不与左右状态信息互相推挤） */
(function fillCopyright() {
  const app = document.getElementById("app");
  const footer = document.getElementById("appFooter");
  if (app && footer && footer.parentElement !== app) app.appendChild(footer);
  const txt = `© ${new Date().getFullYear()} 贵州雏阳生态环保科技有限公司`;
  const el = document.getElementById("appCopyright");
  if (el) el.textContent = txt;
  const fig = document.getElementById("figCopyright");
  if (fig) fig.textContent = txt;
})();
  ...
  try {
    ...
    Object.keys(S.cfg.rowBands).forEach(key => {
      ...
      const stale = arr.some(sg => sg.no != null && !nosNow.has(String(sg.no)))
        ...
        || !bandSegsValid(arr, res.total_km);   // 换线后点集可能相同，旧边界不覆盖新里程即失效
      if (stale) {
...
/* ---------------- 侧栏 ---------------- */
function group(title, bodyHtml, open = true, id = "", ok = null) {
  // 折叠状态记忆：用户手动展开/折叠后，重建侧栏（导入图层等）不重置
  S.__grp = S.__grp || {};
  const closed = Object.prototype.hasOwnProperty.call(S.__grp, title) ? S.__grp[title] : !open;
  // ok：就绪状态徽章（null=该组无状态语义，不显示）
  const state = ok == null ? "" : `<span class="gstate ${ok ? "on" : ""}" title="${ok ? "已就绪" : "待配置"}"></span>`;
  return `<section class="group ${closed ? "closed" : ""}" data-g="${esc(title)}" ${id ? `id="${id}"` : ""}>
    <div class="ghead"><span class="arrow">▾</span><span class="gtitle">${esc(title)}</span>${state}</div>
...
function buildSidebar() {
  ...
  const html = [
    ...
    group("剖面（地图绘制）", `
      ...
      <div class="ds-stat">线 ${pr0.drawLine.length} 顶点 · 点 ${pr0.drawPts.length} 个<span title="在地图工具栏选择「✏ 画断面线」「📍 布点」绘制，归属当前剖面">ⓘ</span></div>`, true, "", lineOk),
    ...
    group("导入线/点", `
      ...
      <div id="impList"></div>`, false, "", impOk),
...
/* band 快照几何校验：首尾覆盖全程、单调衔接、不越界。
   换线/重排后点集可能不变（no 集合相同），旧快照边界不再覆盖当前里程范围即失效——
   失效快照若被地形着色沿用，会出现色带行正常而地形表层缺失/错乱 */
function bandSegsValid(arr, totalKm) {
  if (!Array.isArray(arr) || !arr.length) return false;
  const eps = Math.max(totalKm * 1e-4, 1e-4);
  if (+(arr[0].a ?? 0) > eps) return false;
  if (+(arr[arr.length - 1].b ?? 0) < totalKm - eps) return false;
  for (let i = 0; i < arr.length; i++) {
    const sg = arr[i];
    if (sg.a == null || sg.b == null) return false;
    if (sg.a < -eps || sg.b > totalKm + eps || sg.b <= sg.a - eps) return false;
    if (i > 0 && sg.a < arr[i - 1].b - eps) return false;
  }
  return true;
}
...
function refreshBandTexts(key) {
  ...
  segsB.forEach(sg => {
    ...
    if (v) sg.text = v;
    // 段元数据跟随锚点格刷新：表格改土种/切编码方案后，色带行与地形表层色同源同步
    const sgm = (S.result.segments || []).find(q => String(q.no) === String(sg.no));
    if (sgm) {
      if (sgm.color) sg.color = sgm.color;
      if (sgm.tl) sg.tl = sgm.tl;
      if (sgm.yl) sg.yl = sgm.yl;
      if (sgm.tz) sg.tz = sgm.tz;
    }
  });
}
  ...
  tbl.querySelectorAll("td input").forEach(inp => {
    ...
    inp.oninput = () => {
      ...
      inp.__t = setTimeout(() => {
        ...
        // 编辑影响土种名/编号时，其余 band 段的 text/色同步（保留边界几何）——地形表层色随之更新
        Object.keys(S.cfg.rowBands || {}).forEach(refreshBandTexts);
        if (S.result) renderFigure(S.result);
        computeDebounced();
      }, 600);
    };
...
function renderFigure(res) {
  ...
  const terrSegs = (() => {
    ...
    if (cb && bandSegsValid(cb, totalKm)) return cb.map(sg => ({ a: Math.max(0, sg.a ?? 0), b: Math.min(totalKm, sg.b ?? totalKm), color: sg.color }));
    return segs.map((s, i) => ({ a: B[i], b: B[i + 1], color: s.color }));   // 快照失效时退回逐格着色，保证地形不缺色
  })();
  const defs = el("defs", {});
  ...
  if (terrStyle === "classic") {
    ...
    // 表层带下缘钳制到基线：低洼段地形贴近 X 轴时，地形y+D 会越过轴线
    // 画到轴下方（穿透），钳制后色带贴地形至多到基线，低处厚度随之变薄
    const capY = ypx(dispBase);
    const bandY = (p) => Math.min(ypx(p[1]) + D, capY);
    ...
    segRange.forEach(sg => {
      ...
      pts.forEach(p => d += ` L ${xpx(p[0])} ${bandY(p)}`);
    ...
    segRange.forEach((sg, i) => {
      const pts = segPts(sg.a, sg.b);
      if (pts[pts.length - 1][0] - pts[0][0] < 0.3) return;
      let d = `M ${xpx(pts[0][0]) - 0.7} ${bandY(pts[0])}`;
      pts.forEach(p => d += ` L ${xpx(p[0])} ${bandY(p)}`);
...
function onDragDblClick(svg, st, xy) {
  ...
  if (st.type === "bound" && Array.isArray(c.boundsOverride)) {
    ...
    // 双击行内分段边界：合并相邻两段（保留左段文字，元数据保留左段优先，保段色/锚点不失联）
    const segsB = (c.rowBands || {})[st.bk];
    if (!Array.isArray(segsB) || st.bkI < 1 || st.bkI >= segsB.length) return;
    const L = segsB[st.bkI - 1], R = segsB[st.bkI];
    segsB.splice(st.bkI - 1, 2, { a: L.a, b: R.b, text: L.text || R.text || "", color: L.color || R.color, no: L.no ?? R.no, tl: L.tl || R.tl, yl: L.yl || R.yl, tz: L.tz || R.tz });
    ...
    segsB.splice(st.bkI, 1, { ...sg, b: v }, { ...sg, a: v });   // 拆分两段继承原段元数据（色/锚点）
    const rowLabel = (c.rows.find(r => r.key === st.bk) || {}).label || st.bk;
...
async function refreshMap() {
  if (!S.soil.meta || !S.dem.meta) { $("mapStatus").textContent = "请先在「数据源」组装载土壤图与 DEM"; return; }
...
function showSrcPicker() {
  if (!S.soil.meta || !S.dem.meta) return log("请先在「数据源」组装载土壤图与 DEM", "err");
  const old = $("srcPick");
  if (old) old.remove();
  ...
  ov.innerHTML = `<div style="background:var(--panel);border-radius:12px;padding:20px 24px;min-width:430px;max-width:460px;box-shadow:0 16px 48px rgba(20,16,12,.32);border:1px solid var(--border)">
    ...
    <div class="row"><label>线套</label><span class="fill"><select id="srcLineSel" style="width:100%">${
      lineImps.map(i => `<option value="${i.id}" ${i.id === S.genLineId ? "selected" : ""}>${esc(i.name)}</option>`).join("") || '<option value="">（无，请先在「导入线/点」组装载）</option>'
    }</select></span></div>
    <div class="row"><label>点套</label><span class="fill"><select id="srcPtsSel" style="width:100%">${
      ptsImps.map(i => `<option value="${i.id}" ${i.id === S.genPtsId ? "selected" : ""}>${esc(i.name)}</option>`).join("") || '<option value="">（无，请先在「导入线/点」组装载）</option>'
    }</select></span></div>
...
async function generateWithMode(mode, profileId) {
  ...
  if (mode === "draw") {
    ...
    if (!li || !pi) return log("尚未选择导入线套/点套：请在「导入线/点」组装载，或弹出层中选择", "err");
    try {
